...
class Solution {
    public int[] searchRange(int[] nums, int target) {
        int bg = findFirst(nums, target);
        int end = findFirst(nums, target + 1) - 1;
        int n = nums.length;
        if (bg < n && nums[bg] == target) {
            return new int[] { bg, end };
        }
...
```
全局变量维护当前遍历到哪一个下标了
因为 c == '[' 要进入 dfs，所以 dfs 返回的类型是 String 的
```
...
```
单调递减栈，如果没有更高温度就用 0 代替（数组刚好默认也是 0）
```
...
```
每个 partition 里面严格分为 < pivot 和 > pivot
...
class Solution {
    public int findKthLargest(int[] nums, int k) {
        int temp = 10010;
        int n = nums.length;
        int[] cnt = new int[2 * temp];
        for (int i = 0; i < n; i++) {
            cnt[nums[i] + temp]++;
        }

        for (int i = 2 * temp - 1; i >= 0; i--) {
            k = k - cnt[i];
...
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            maxCnt = Math.max(maxCnt, cnt.get(nums[i]));
        }
...
```
因为在 for 循环里面遍历过 n - 1 了，所以出了 for 循环的话，cur 就一定 >= n - 1，就是返回 true
```
...
可以模拟一下过程
cur 是当前（最远）位置
maxRight 是跳到 cur 位置后，继续遍历 i + nums[i] 能到的最远位置

就是 i <= cur，既然可以跳到 cur，那么就能跳到某个 i 使 maxRight > cur，就是更远的位置

当 i > cur 的时候，就必须要跳一下了（跳到 maxRight）
```
...
```
必须是先 更新 end，再在 if (i == end) 的时候 ans.add(...) 

如果是先判断 i > end，再 end = max(...) 的话，最后一个会记录不到
```
...
求最大值时，除了 dp[0]为 0，其他都初始化为无穷小 -0x3f3f3f3f

求最小值时，除了 dp[0]为 0，其他都初始化为无穷大 0x3f3f3f3f
...
```
第一轮结束后：所有 dp[j] 都已经是“合法值”了

不再是 Integer.MAX_VALUE

所以初始化不用 Integer.MAX_VALUE / 2
```
...
注意初始值是：Integer.MAX_VALUE / 2 （参考上面 279. 完全平方数 的讲解）
```
...
class Solution {
    public int maxProduct(int[] nums) {
        ...
        for (int i = 1; i < n; i++) {
            int x = nums[i];
            dpMax[i] = Math.max(x, Math.max(x * dpMax[i - 1], x * dpMin[i - 1]));
            dpMin[i] = Math.min(x, Math.min(x * dpMax[i - 1], x * dpMin[i - 1]));
...
```
dpMax[i] 表示以 i 结尾的最大值
dpMin[i] 表示以 i 结尾的最小值

前一个最小值可能在这一次成为最大值，所以要同时维护最大值和最小值
```
...
```
看成 01 背包，每个体积和价值是相等的
如果最大价值 < sum 的话，就说明无法恰好装满
又因为每个体积和价值是相等的，所以 dp[i] 不可能 > i

不要看成恰好装满问题，太复杂了，直接看成 01 背包问题就好，当 dp[i] == i 就表示恰好装满
```
...
```
括号匹配，典型的用栈
先 push 一个哨兵进去，如果 stk 空了就说明哨兵也 pop() 了
